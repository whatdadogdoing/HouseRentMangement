    ...
    namespace HouseRentManagement
    {
        ...
        {
            DBQLChungCu dbChungCu = new DBQLChungCu();
            public CondoAdmin()
            {
                InitializeComponent();
            }
            ...
            public void BindGrid(List<CANHO> listCanHo, List<LOAICANHO> listLoaiCanHo)
            {
                ...
                for (int i = 0; i < Math.Max(listCanHo.Count, listLoaiCanHo.Count); i++)
                {
                    int index = dgvCondoAdmin.Rows.Add();

                    if (i < listCanHo.Count)
                    {
                        dgvCondoAdmin.Rows[index].Cells[0].Value = index + 1;
                        dgvCondoAdmin.Rows[index].Cells[1].Value = listCanHo[i].MaCanHo;
                        dgvCondoAdmin.Rows[index].Cells[2].Value = listCanHo[i].MaTang;
                        dgvCondoAdmin.Rows[index].Cells[3].Value = listCanHo[i].MaChuSoHuu;
                        dgvCondoAdmin.Rows[index].Cells[4].Value = listCanHo[i].MaLoai;
                        dgvCondoAdmin.Rows[index].Cells[5].Value = listCanHo[i].TinhTrang;
                        // Các trường dữ liệu khác từ bảng "CANHO"
                    }

                    if (i < listLoaiCanHo.Count)
                    {                  
                        dgvCondoAdmin.Rows[index].Cells[6].Value = listLoaiCanHo[i].TenLoai;
                        dgvCondoAdmin.Rows[index].Cells[7].Value = listLoaiCanHo[i].DienTich;
                        dgvCondoAdmin.Rows[index].Cells[8].Value = listLoaiCanHo[i].SoPhongNgu;
                        dgvCondoAdmin.Rows[index].Cells[9].Value = listLoaiCanHo[i].GiaThueMacDinh;
        ...
        private void txtBoxSearch_TextChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                BindGrid(filteredCanHo, filteredLoaiCanHo);
            }
        }
    }
}
