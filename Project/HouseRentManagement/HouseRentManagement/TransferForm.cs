...
namespace HouseRentManagement
{
    public partial class TransferForm : DevExpress.XtraEditors.XtraForm
    {
        ...
        private bool checkIfValidDateTime()
        {
            ...
            // Kiểm tra ngày giờ kết thúc không được lớn hơn quá 2 ngày
            if (endDate > startDate.AddDays(2))
            {
                MessageBox.Show("The end date and time cannot be more than 2 days greater than the start date and time!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
            if(startDate < minstart || startDate > maxstart || endDate < minend || startDate > maxend)
            {
        ...
        private string GetNextReceiptNumber()
        {
            ...
            {
                conn.Open();
                string query = "SELECT MAX(CAST(SUBSTRING(SoPhieuChuyenDo, 3, 3) AS INT)) FROM PHIEUCHUYENDO";
                using (SqlCommand cmd = new SqlCommand(query, conn))
                {
                    ...
                    if (result != DBNull.Value)
                    {
                        ...
                        return "CD" + nextNumber.ToString("D3");
                    }
                    else
                    {
                        return "CD000";
        ...
        private void btnSend_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string insertQuery = @"INSERT INTO PHIEUCHUYENDO 
                (SoPhieuChuyenDo, NgayLap, NoiDung, TrangThai, BDChuyen, KTChuyen, MaCanHo) VALUES
                (@SoPhieu, @NgayLap, @NoiDung, N'Đang chờ xử lý', @BDChuyen, @KTChuyen, @MaCanHo)";
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(insertQuery, conn))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@BDChuyen", beginDate);
                        cmd.Parameters.AddWithValue("@KTChuyen", endDate);
                        cmd.Parameters.AddWithValue("@MaCanHo", condoId);
                        ...
                        if (rowsAffected > 0)
                        {
                            MessageBox.Show("Transfer form has been sent!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            txtBoxCondoID.Clear();
                            txtResCardID.Clear();
                            txtBoxContent.Clear();
                            setUpDTP();
